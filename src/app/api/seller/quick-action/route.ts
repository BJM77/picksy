...
export async function GET(request: NextRequest) {
    try {
        ...
        return new NextResponse(`
            <html>
                <head>
                    <title>Action Confirmed | Picksy.au</title>
                ...
                <body class="bg-slate-50 flex items-center justify-center min-h-screen p-4 font-sans text-center">
                    <div class="bg-white p-8 rounded-3xl shadow-xl max-w-sm w-full border border-slate-100">
                        ...
                        <a href="https://picksy.au/seller/dashboard" class="inline-block w-full bg-slate-900 text-white font-bold py-4 rounded-2xl hover:bg-slate-800 transition-all">Go to Dashboard</a>
                    </div>
                </body>
            </html>
        `, {
